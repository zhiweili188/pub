...
!function($) {
    ...
    $.fn.valid=function(object,options){
        ...
        if(myoptions !=null && validationError){
            ...
            $(".errmsg").after('<span id="validerrmsg" class="help-block" style="color: #FF0000;">'+myoptions+'</span>');
   ...
   $.fn.validation.defaults = {
        validRules : [
            ...
            {name: 'username',validate:function(value){return(/^[a-zA-Z]{1}[0-9a-zA-Z_]{1,}$/.test(value)==false);},defaultMsg:"只能输入字母、数字，并且字母开头。"},
            {name: 'passwd',validate:function(value){return(/^\w{6,20}$/.test(value)==false);},defaultMsg:"密码格式错误,长度在6-20之间，只能包含字符、数字和下划线。"},
            {name: 'mobile',validate:function(value){return(/^1\d{10}$/.test(value)==false);},defaultMsg:"错误的手机号码格式。"},
            {name: 'isCardNo',validate:function(value){return(/(^\d{15}$)|(^\d{18}$)|(^\d{17}(\d|X|x)$)/.test(value)==false);},defaultMsg:"身份证输入不合法。"},
                ...
                $(obj).find('input, textarea, select').each(function(){
                    ...
                    if (valid){
                        if ($.inArray('required',valid)>=0){
                        	...
                        	var lb = controlGroup.find("label");
                        	lb.append("<font color='red'>  *</font>");
                        }
                    };
                }); 
            };
